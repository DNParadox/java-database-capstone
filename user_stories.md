# 📋 Smart Clinic – All User Stories

> [!IMPORTANT]
> This file consolidates all user stories for the **Admin**, **Doctor**, and **Patient** roles in the Smart Clinic system.
> Each role defines clear, goal-driven workflows that support data integrity, role-based access, and platform usability.

---

## Table of Contents

- [Role: Admin](#role-admin)
  - [Story 1 – Admin Login](#admin-story-1--admin-login)
  - [Story 2 – Admin Logout](#admin-story-2--admin-logout)
  - [Story 3 – Add Doctor](#admin-story-3--add-doctor)
  - [Story 4 – Delete Doctor Profile](#admin-story-4--delete-doctor-profile)
  - [Story 5 – View Monthly Appointments](#admin-story-5--view-monthly-appointments)
- [Role: Doctor](#role-doctor)
  - [Story 1 – Doctor Login](#doctor-story-1--doctor-login)
  - [Story 2 – Doctor Logout](#doctor-story-2--doctor-logout)
  - [Story 3 – View Appointment Calendar](#doctor-story-3--view-appointment-calendar)
  - [Story 4 – Mark Unavailability](#doctor-story-4--mark-unavailability)
  - [Story 5 – View Patient Details](#doctor-story-5--view-patient-details)
- [Role: Patient](#role-patient)
  - [Story 1 – View Doctors Without Logging In](#patient-story-1--view-doctors-without-logging-in)
  - [Story 2 – Patient Sign-Up](#patient-story-2--patient-sign-up)
  - [Story 3 – Patient Login](#patient-story-3--patient-login)
  - [Story 4 – Book Appointment](#patient-story-4--book-appointment)
  - [Story 5 – View Upcoming Appointments](#patient-story-5--view-upcoming-appointments)

---

## Role: Admin

**As an** administrator
**I need** core capabilities to manage users and monitor operations
**So that** I can ensure system reliability, user control, and compliance.

---

### Admin Story 1 – Admin Login

_As an admin, I want to log into the portal with my credentials, so that I can securely manage the system._

```gherkin
Given I am on the login page
When I enter valid admin credentials
Then I should be redirected to the admin dashboard
And my session should be authenticated securely
```

---

### Admin Story 2 – Admin Logout

_As an admin, I want to log out of the portal, so that I can end my session securely._

```gherkin
Given I am logged in as an admin
When I click the logout button
Then I should be logged out and redirected to the login screen
And my session should be terminated
```

---

### Admin Story 3 – Add Doctor

_As an admin, I want to add doctors to the system, so that they can manage appointments._

```gherkin
Given I am on the "Add Doctor" form
When I submit valid doctor information
Then the doctor profile should be saved to MySQL
And be visible in the doctor list
```

---

### Admin Story 4 – Delete Doctor Profile

_As an admin, I want to delete a doctor's profile, so that I can manage active practitioners._

```gherkin
Given a doctor has no future appointments
When I click "Delete" on their profile
Then the record should be removed from the system
And no appointments should reference that doctor
```

---

### Admin Story 5 – View Monthly Appointments

_As an admin, I want to run a MySQL stored procedure to get monthly appointment counts, so that I can track platform usage._

```gherkin
Given I have access to the MySQL CLI
When I run the appointment_stats procedure
Then I should receive a list of appointment counts grouped by month
```

---

## Role: Doctor

**As a** doctor
**I need** access to my appointment calendar and patient information
**So that** I can deliver efficient care and manage my availability.

---

### Doctor Story 1 – Doctor Login

_As a doctor, I want to log into the portal, so that I can manage my appointments._

```gherkin
Given I am registered as a doctor
When I enter valid credentials on the login page
Then I should be redirected to the doctor dashboard
And I should see my upcoming schedule
```

---

### Doctor Story 2 – Doctor Logout

_As a doctor, I want to log out of the portal, so that I can protect my session and personal data._

```gherkin
Given I am logged in as a doctor
When I click the logout button
Then I should be logged out and redirected to the homepage
And my session should be terminated
```

---

### Doctor Story 3 – View Appointment Calendar

_As a doctor, I want to view my appointment calendar, so that I can organize my work schedule._

```gherkin
Given I am logged in
When I navigate to the calendar
Then I should see a visual representation of my appointments for the day/week/month
```

---

### Doctor Story 4 – Mark Unavailability

_As a doctor, I want to mark specific times as unavailable, so that patients cannot book during those periods._

```gherkin
Given I am logged in
When I mark certain time slots as unavailable
Then the system should block those slots from being booked by patients
```

---

### Doctor Story 5 – View Patient Details

_As a doctor, I want to view patient details for each appointment, so that I can be better prepared for consultations._

```gherkin
Given I have scheduled appointments
When I click on a specific appointment
Then I should see the patient's name, reason for visit, and basic history
```

---

## Role: Patient

**As a** patient
**I need** the ability to explore, book, and manage appointments
**So that** I can receive medical care efficiently and securely.

---

### Patient Story 1 – View Doctors Without Logging In

_As a patient, I want to view a list of doctors without logging in, so that I can explore my options before registering._

```gherkin
Given I access the doctor listing page
When I am not logged in
Then I should see names, specialties, and contact info of available doctors
```

---

### Patient Story 2 – Patient Sign-Up

_As a patient, I want to sign up using my email and password, so that I can book appointments._

```gherkin
Given I am on the signup page
When I submit valid registration details
Then my account should be created
And I should be redirected to the login page
```

---

### Patient Story 3 – Patient Login

_As a patient, I want to log into the portal, so that I can manage my bookings._

```gherkin
Given I have a registered account
When I enter valid credentials
Then I should be redirected to my dashboard
And gain access to appointment management features
```

---

### Patient Story 4 – Book Appointment

_As a patient, I want to book an hour-long appointment, so that I can consult with a doctor._

```gherkin
Given I am logged in
When I select a doctor, date, and time
Then the system should verify availability
And confirm the booking
```

---

### Patient Story 5 – View Upcoming Appointments

_As a patient, I want to view my upcoming appointments, so that I can prepare in advance._

```gherkin
Given I am logged in
When I visit the "My Appointments" section
Then I should see a list of all upcoming appointments with relevant details
```

---

> _Last updated: April 4, 2026_

